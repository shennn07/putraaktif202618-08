...
$batasKembali  = date('Y-m-d', strtotime('+3 days'));

mysqli_begin_transaction($koneksi);
try {
    $stmt = mysqli_prepare($koneksi,
        "UPDATE borrowings SET status='borrowed', borrow_date=?, due_date=? WHERE id=?");
    mysqli_stmt_bind_param($stmt, 'ssi', $tanggalPinjam, $batasKembali, $id);
    mysqli_stmt_execute($stmt);

    $stmt = mysqli_prepare($koneksi, "UPDATE books SET status='borrowed' WHERE id=?");
    mysqli_stmt_bind_param($stmt, 'i', $transaksi['book_id']);
    mysqli_stmt_execute($stmt);

    mysqli_commit($koneksi);
    setAlert('success', 'Peminjaman dikonfirmasi. Batas kembali: ' . formatTanggal($batasKembali) . '.');
} catch (Exception $e) {
    mysqli_rollback($koneksi);
    setAlert('danger', 'Gagal mengkonfirmasi peminjaman.');
}
